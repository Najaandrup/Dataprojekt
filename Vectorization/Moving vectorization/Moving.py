...
def vectorize_and_plot(tsv_file, pod_file, vector_length, window_size_avg, window_size_var):
    # Load position data
    ...
    for _, row in df.iterrows():
        ...
        if len(signal50) == 0:
            continue  # Skip if signal slice is invalid
        ...
        for i in range(vector_length):
            x = i * ((end - start) / vector_length)
            x = int(x)


            start_index_avg = max(int(start + x - (window_size_avg // 2)), 0)
            end_index_avg = min(int(start + x + (window_size_avg // 2)), len(normalized))
            ...
            start_index_var = max(int(start + x - (window_size_var // 2)), 0)
            end_index_var = min(int(start + x + (window_size_var // 2)), len(normalized))
    ...
    plt.title(f"Moving Average - Normalized with n={vector_length}, w={window_size_avg}")
    plt.xlabel("Window Index")
    plt.ylabel("Normalized Mean")
    plt.legend()
    plt.grid(True)
    mean_plot_name = f"{base_name}_mean_n{vector_length}_w{window_size_avg}.png"
    plt.savefig(mean_plot_name, dpi=300)
    plt.close()
    ...
    plt.title(f"Moving Variation - Normalized with n={vector_length}, w={window_size_var}")
    plt.xlabel("Window Index")
    plt.ylabel("Variance")
    plt.legend()
    plt.grid(True)
    var_plot_name = f"{base_name}_var_n{vector_length}_w{window_size_var}.png"
    plt.savefig(var_plot_name, dpi=300)
    plt.close()
...
if __name__ == '__main__':
    ...
    vectorize_and_plot(tsv_file, pod_file, vector_length, window_size_avg, window_size_var)
